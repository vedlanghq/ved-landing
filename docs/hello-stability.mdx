...
Instead of a traditional `Hello World` that prints to standard output and immediately dies, our introductory example demonstrates Lexum's core promise: **Stability**.
...
state {
    desiredMessage: String = "Hello Lexum"
...
1. **Initial Boot:** You run `lexum runtime start hello.lxm`.
2. **Snapshot Load:** The runtime checks for a `.snapshot.json`. Since none exists, it boots fresh.
...
The moment you do, Lexum's next background tick evaluates the `predicate`. It suddenly returns `false`. The runtime wakes up, pulls the `CreateFile` transition, and restores the file immediately.
