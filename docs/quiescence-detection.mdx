...
Lexum solves this using a formally defined concept of **Quiescence (Stable State)** coupled with programmatic Oscillation Detection.
...
Once Quiescence is achieved, the Lexum Scheduler places the Domain into a deeply optimized "Sleep" mode. It consumes virtually zero CPU cycles until a new asynchronous message or external event arrives in its mailbox, breaking the quiescence.

## Oscillation Detection

If a Domain repeatedly transitions back and forth without achieving Quiescence rapidly, the Lexum runtime detects it as **State Oscillation**.
...
This guarantees that poorly written Lexum programs fail safely and quietly, rather than burning planetary resources trying to achieve the mathematically impossible.
