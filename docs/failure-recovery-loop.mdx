...
3.  **The Degradation:** The goal is instantly broken. The Ved runtime wakes up. It evaluates the transitions. *Both* `ExecuteFailover` and `TriggerPagerDuty` are now valid. 
