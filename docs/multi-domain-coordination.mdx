...
Most developers are terrified of "Eventual Consistency" (the idea that system A knows something, but system B won't find out for a few milliseconds). Ved leans into it. The `AppProvisioner` doesn't actually know if the database is currently running; it only knows that _its internal state_ says `db_ready = true`. This forces developers to design highly resilient systems that can handle being out of sync temporarily.
