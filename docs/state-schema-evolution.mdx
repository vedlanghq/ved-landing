...
Lexum is designed for **long-lived evolving systems**. In Lexum, program execution is intertwined with its Durable Snapshots. Because a Lexum Domain can effectively run for months or years via mathematical pauses and resumptions, the schema of its underlying state will eventually need to change.
...
## Lexum's Built-In Versioning Model

Lexum treats **Schema Evolution as a first-class language capability**.

Instead of over-automating migrations with "black box" magic, Lexum implements a pragmatic architecture:
