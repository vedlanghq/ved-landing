...
The architectural path to Lexum was not a straight line. Creating a language that strictly distances business logic from system effects while retaining high usability required heavy trade-offs.
...
Lexum exists because we wanted those invariants baked directly into the compiler AST, rather than relying on code-review conventions.

## The Explicit Emits

Earlier versions of the Lexum philosophy experimented with hiding the `Effect Boundary`. What if the developer just typed `API.call()`, and under the hood, the compiler sliced the function in half natively?
...
Because Lexum operates entirely on continuous convergence reconciliation, an unhandled network error is not an "Exception." It is simply data. Data that must be saved to the memory snapshot, allowing the Control Loop to mathematically degrade, retry, or escalate on the next tick.
