...
In Lexum, `Lexum lint` and `Lexum verify` are not tools that complain about line lengths or variable names. They are powerful mathematical verification engines designed to mathematically guarantee that your control loops are safe, deterministic, and idempotent before they are deployed.
...
When you run `Lexum lint`, the CLI performs three distinct checks:
...
You can configure the strictness of the linter in your `Lexum.toml` file or bypass specific lines using inline Pragmas. However, a bypassed lint rule is permanently attached to the compiled artifact's telemetry profile.

## `Lexum verify`

While `lint` checks the structural purity of the code execution, `Lexum verify` acts on the **orchestration topology**. It requires your entire multi-domain project workspace.

When you run `Lexum verify`, it ensures:
...
In a CI/CD pipeline, `Lexum check` is used for fast parsing, but `Lexum verify` is the definitive gating function before allowing a deployment release.
