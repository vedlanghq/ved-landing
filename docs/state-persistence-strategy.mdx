...
If Lexum's only promise was "determinism", an in-memory graph would be perfectly fine. But because Lexum must orchestrate long-lived systems that inevitably survive machine failures, the state inside a Lexum application must seamlessly persist to durable storage.
...
Lexum uses a radically optimized **Snapshot-First Persistence Layer** backed by Copy-On-Write data structures.
...
1. **Instant Cold Starts:** When a Lexum Runtime node crashes and restarts, it does not replay history. It loads the exact binary representation of the most recent atomic snapshot from disk instantly.
