...
Lexum solves this paradox gracefully via the **Effect Execution Boundary**.

## The Hard Partition

The Lexum Runtime is mathematically split into two zones:
...
The `Effect Adapter` module within the Lexum Runtime lives here. It is fully aware of the messy real world. It has TCP sockets, file system access, and wall-clock time.
...
3. The _Impure Zone_ (The Effect Adapter) picks up the packet and actually initiates the TCP request to the AWS API.
