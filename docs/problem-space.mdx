...
When engineers write imperative scripts (in Python, Go, or shell) to manage infrastructure or orchestrate services, they are manually describing _how_ a machine should transition from point A to point B.

1. Do X
2. Check if Y is ready
3. If Y is ready, do Z
4. _(What if the network fails here?)_
5. _(What if X was successful, but the database says no?)_
...
Tools like Terraform or Kubernetes manifests allow developers to declare _what_ they want (e.g., "I want 5 replicas"), but they use domain-specific configuration languages mapping to hidden, hard-coded backend controllers written in Go. They are not highly expressive general-purpose programming languages.
...
The problem space of Lexum is the taming of chaotic operational logic into safe, predictable system evolution.
