...
Lexum's design is heavily documented through Architecture Decision Records. These are formal snapshots of the philosophical trade-offs made during the founding of the language.
...
- **Decision:** Lexum must ship with a proprietary _Virtual Control Runtime_ that executes purely deterministic slices.
...
- **Decision:** Lexum structurally forbids shared memory. All state is perfectly partitioned into Domains. Domains communicate exclusively through asynchronous mailboxes.
...
- **Decision:** Lexum bakes Authority Scope into the formal Type System (`Domain<Workspace>` vs `Domain<Tenant>`).
