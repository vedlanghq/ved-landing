...
While the v0.1 release of Lexum implies a single-node Virtual Control Runtime, the language semantics (isolated domains, asynchronous mailboxes, copy-on-write snapshots, and deterministic execution) were custom-designed to support a globally distributed runtime.

If a language relies on shared memory mutexes, distributing it across a network becomes a nightmare of race conditions and distributed locks. Because Lexum enforces strict Domain isolation, distributing Lexum is mathematically elegant.

## The Target Topology

A future distributed Lexum cluster consists of:
...
Because Lexum is designed for safe infrastructure orchestration, **Lexum always chooses Consistency (Determinism) over Availability**.
