...
Because the Lexum Runtime mandates that all mutations happen via perfectly pure mathematical Transitions and all physical side-effects are captured as formal Events in an immutable log, **Lexum natively supports time-travel replay debugging.**
...
3. They spin up `Lexum runtime start --replay` locally.
