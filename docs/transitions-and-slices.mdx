...
In Lexum, you do not write long, monolithic scripts or functions that run indefinitely.
...
A Transition in Lexum is a logical workflow designed to move a Domain from its current state closer to its Goal state. However, a Transition is not allowed to run as an unbounded, blocking function.

The Lexum Compiler forces you to break Transitions into **Execution Slices**.
...
By enforcing this structure, the Lexum runtime provides an incredibly powerful guarantee: **Crash-Resilient Forward Progress.**

The runtime works like this:

1. Lexum executes `validateInputs`.
2. Upon successful completion, the runtime automatically checkpoints the exact state of the Domain to persistent storage.
3. Lexum executes `provisionNetwork`.
4. Power is lost, and the hardware node crashes.

When the Lexum runtime restarts, it looks at the persistent snapshot. It does **not** restart the transition from the beginning. It completely bypasses `validateInputs` and resumes execution smoothly exactly at `provisionNetwork`.

Unlike traditional thread interruption, Lexum will **never preempt a Domain in the middle of a slice.** Preemption, state snapshotting, and rescheduling across a cluster only ever happen cleanly exactly on slice boundaries.

## Compiler Limits

To ensure that malicious or badly written code does not monopolize the deterministic scheduler, the Lexum Compiler statically enforces rules on Slices at compile-time:
