...
Because Lexum operates deeply within the paradigm of Control Loops and Domains rather than simple monolithic CPU-bound execution threads, standard UNIX observability commands (like `top` or `ps`) are not expressive enough.

The Lexum CLI offers built-in operational probes designed specifically to peer inside the Virtual Control Runtime.

## Live Cluster Inspection

### `Lexum get domains [--status <status>]`

Returns an aggregate list of all registered state domains actively hosted on the local runtime.

- _Example Output:_ Shows the logical ID, current active version, total accumulated mail queue depth, and internal lifecycle status (`ACTIVE`, `DEGRADED`, `FAULTED`).

### `Lexum get goals <domain_id>`

Displays the current resolution status of all goals running within a specific Domain.

- _Example Output:_ Tracks the current Evaluation Predicate boolean, the measured Drift Time, and the priority ranking configured by the Deterministic Scheduler.

## Execution Analysis

### `Lexum trace effect <effect_id>`
...
An interactive terminal dashboard mirroring the philosophical interface of UNIX `top`, but tailored for the Lexum Scheduler.
...
Because of snapshot persistence and the immutable message journal, Lexum logging is natively deterministic.

### `Lexum logs query "priority >= 50 && domain == 'PaymentGateway'"`
