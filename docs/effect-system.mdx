...
The Ved Runtime—which operates _outside_ the mathematical boundary of the Domain—reads the intent, makes the actual chaotic HTTP request out to AWS/Azure, and waits for a response. Because this happens in the background runtime, it does not block other Domains from making forward progress.
