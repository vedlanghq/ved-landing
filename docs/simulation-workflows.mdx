...
To test these scenarios locally, without spinning up expensive cloud resources, Lexum offers the **Deterministic Replay Engine**.

## The Time-Travel Harness

Because Lexum's business logic is mathematically isolated from system effects via the Effect Boundary, it is trivial to replace the actual `Network Adapter` with a `Test Adapter` during simulation.

More importantly, the Test Adapter has the ability to manipulate **Logical Time**.

When running `Lexum simulate`, developers can orchestrate scenarios spanning days or weeks, executing them practically instantly.
