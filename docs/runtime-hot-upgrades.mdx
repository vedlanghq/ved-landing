...
For web servers, this is fine because HTTP requests are short-lived. But in Lexum, a single Domain might be actively orchestrating a 7-day database migration workflow. Draining that connection is impossible.

Instead, Lexum is designed to support **Deterministic Hot Upgrades**.

## The Execution Pause

Because Lexum executes in highly isolated Deterministic Slices, a Lexum node never has "hanging threads" blocking the CPU indefinitely.
